...
class AE_Decoder_Trainer():
    ...
    def compute_loss(self, imgs, lbls, stats, recon_lambda=1, recon_zero_lambda=1, \
        ...
        z_dim = self.ae.module.iin_ae.z_dim
        z_force = self.lbls_to_att_fn(lbls).float().to(imgs.get_device())
        z_in = torch.normal(0, 1, size=(len(imgs), z_dim)).to(imgs.get_device())
        z_in[:, :z_force.shape[1]] = z_force
        #if force_hardcode:
        ...
        #else:
        imgs_recon = self.ae.module.decode(self.ae.module.replace(z_in))
    ...
    def eval(self, test_dloader, logger=None, force_hardcode=False):
        ...
        with torch.no_grad():
            for (imgs, lbls) in tqdm(test_dloader, desc="Evaluation"):
                ...
                imgs_zero = self.ae.module.decode(torch.cat((z_force, torch.zeros_like(z_in[:, self.num_att_vars:])), 1))
