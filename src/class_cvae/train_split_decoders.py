...
if __name__ == "__main__":
    ...
    total_params += sum(p.numel() for p in class_decoder.parameters() if p.requires_grad)
    ...
    class_decoder.apply(init_weights)
    ...
    lpips_loss_fn = LPIPS()
    class_loss_fn = nn.CrossEntropyLoss()
    params = list(encoder.parameters()) + list(decoder.parameters()) + list(class_decoder.parameters())
    ...
    for epoch in range(args.epochs):
        ...
        for imgs, lbls in tqdm(train_dloader, desc="Training"):
            ...
            out = img_classifier(cls_imgs_recon)

            img_cls_loss = class_loss_fn(out, lbls) * 0.1 + lpips_loss_fn(cls_imgs_recon, imgs) #+ l1_loss_fn(cls_imgs_recon, imgs))
        ...
        save_imgs(imgs, imgs_recon, cls_imgs_recon, logger.get_path())
        ...
        with torch.no_grad():
            for imgs, lbls in test_dloader:
                ...
                recon = class_decoder(z[:, :args.num_class_features])
                out = img_classifier(recon)
                _, preds = torch.max(out, dim=1)
                correct[1] += (preds == lbls).sum().item()

                total += len(imgs)

            logger.log(f"Epoch: {epoch+1} | Test Accuracy Full: {round(correct[0]/total, 4)} | Test Accuracy Class: {round(correct[1]/total, 4)}")

        torch.save(encoder.state_dict(), f"{logger.get_path()}/encoder.pt")
        torch.save(decoder.state_dict(), f"{logger.get_path()}/decoder.pt")
        torch.save(class_decoder.state_dict(), f"{logger.get_path()}/class_decoder.pt")
