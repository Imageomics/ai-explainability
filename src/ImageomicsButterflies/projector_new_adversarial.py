...
def project(
    ...
    *,
    ...
    saved_z = np.random.RandomState(123).randn(1, G.z_dim)
    ...
    for step in range(num_steps):
        ...
        lbls = torch.tensor([target_lbl]).cuda()
        ...
        #loss = #reg_loss * regularize_noise_weight + mse_loss + smooth_loss
...
def run_projection(
    ...
    if save_video:
        ...
        for i in range(len(imgs)):
            ...
            fig = plt.figure(figsize=(target_uint8.shape[1]*2*px, target_uint8.shape[0]*px))
            plt.plot(confs)
            ...
            video.append_data(np.concatenate([np.concatenate([target_uint8, (synth_image* 255).astype(np.uint8), diff_image], axis=1), np.concatenate([conf_graph, heatmap_fig], axis=1)], axis=0))
        video.close()
